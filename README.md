...
OCaml allows to decorate code with attributes, which look like this: `[@foo]`.
These attributes can be attached to expressions, functions, or types to trigger
specific code transformations. For example:
...
PPXs will pick up this attribute at compilation time and do some syntactic
modification to the code. You can think of PPXs as Babel plugins, to establish a
comparison with a similar tool in JavaScript.

If you want to read more about PPXs, [this
article](https://tarides.com/blog/2019-05-09-an-introduction-to-ocaml-ppx-ecosystem/)
provides a great introduction to them.

> [!TIP]
> 
> You might have noticed that these attributes look similar to the ones used to
> define components, that's right! ReasonReact is also a PPX and decorators like
> `[@react.component] are used to generate additional code without having to do
> so manually.

To generate our decoders manually, we will use the PPX included with
`melange-json`, which we already installed in the previous step.
...
Then, we will annotate all types in `Feed.re` with `[@deriving json]`, e.g.:
...
After this, we will need to open the PPX module that exposes the functions to
convert primitive types. Which we can do by adding this line at the top of
`Feed.re`:
...
Now, we can remove the whole `Decode` module as these functions are generated by
the PPX.

Finally, slightly modify the `demoFeed` function to use the newly generated
function:
...
We did a refactor to remove some code and introduce the `melange-json` PPX, but
the behavior didn't change, so open the browser console should keep showing our
debugging object:
